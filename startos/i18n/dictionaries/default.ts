...
const dict = {
  ...
  'An environment variable name may contain only letters, digits and underscores, and may not start with a digit.': 53,
  'The serve arguments must start with a model id.': 54,
  Custom: 23,
  'Environment variables': 43,
  'Extra environment variables for the `vllm serve` process — a HuggingFace token for a gated model, or a `VLLM_*` tuning flag. The package sets `HF_HUB_CACHE`, `PYTHONUNBUFFERED` and `HF_HUB_VERBOSITY`, and a variable named here replaces the one it sets.': 44,
  'Pointing `HF_HUB_CACHE` somewhere other than `/data/models` moves the model cache off the persistent volume, so every start re-downloads the model.': 45,
  Name: 46,
