...
- **Custom** serve arguments are now quoted the way a shell quotes a command line, so \`--foo "a b"\` and \`--bar '{"k": 1}'\` each stay a single argument. Previously the string was split on whitespace and a quoted value could not survive. Quoting is all that is interpreted — a \`$VAR\`, a \`*\` glob, a pipe or a redirection reaches vLLM as written.
- The **Custom** model option now takes a list of environment variables alongside the serve arguments — an \`HF_TOKEN\` for a gated model, or a \`VLLM_*\` tuning flag. They apply to a Custom selection only, so picking a preset clears them.
...
- Los argumentos de arranque **Personalizados** se entrecomillan ahora como en un shell, de modo que \`--foo "a b"\` y \`--bar '{"k": 1}'\` siguen siendo un único argumento. Antes la cadena se dividía por espacios y un valor entrecomillado no sobrevivía. Solo se interpreta el entrecomillado: una \`$VAR\`, un comodín \`*\`, una tubería o una redirección llegan a vLLM tal cual.
- La opción de modelo **Personalizado** ahora acepta una lista de variables de entorno junto a los argumentos de arranque: un \`HF_TOKEN\` para un modelo restringido o un flag de ajuste \`VLLM_*\`. Solo se aplican a una selección personalizada, así que elegir un preajuste las borra.
...
- **Benutzerdefinierte** Serve-Argumente werden jetzt wie in einer Shell in Anführungszeichen gesetzt, sodass \`--foo "a b"\` und \`--bar '{"k": 1}'\` jeweils ein einzelnes Argument bleiben. Zuvor wurde die Zeichenkette an Leerzeichen geteilt und ein Wert in Anführungszeichen ging verloren. Interpretiert werden nur die Anführungszeichen — eine \`$VAR\`, ein \`*\`-Glob, eine Pipe oder eine Umleitung erreicht vLLM unverändert.
- Die Modelloption **Benutzerdefiniert** nimmt jetzt neben den Serve-Argumenten eine Liste von Umgebungsvariablen entgegen — ein \`HF_TOKEN\` für ein zugangsbeschränktes Modell oder ein \`VLLM_*\`-Optimierungs-Flag. Sie gelten nur für eine benutzerdefinierte Auswahl, die Wahl einer Voreinstellung löscht sie also.
...
- **Niestandardowe** argumenty uruchomieniowe są teraz cytowane tak jak w powłoce, więc \`--foo "a b"\` i \`--bar '{"k": 1}'\` pozostają pojedynczym argumentem. Wcześniej ciąg był dzielony po spacjach i wartość w cudzysłowie nie przetrwała. Interpretowane są wyłącznie cudzysłowy — \`$VAR\`, wzorzec \`*\`, potok czy przekierowanie trafiają do vLLM bez zmian.
- Opcja modelu **Niestandardowe** przyjmuje teraz listę zmiennych środowiskowych obok argumentów uruchomieniowych — \`HF_TOKEN\` do modelu z ograniczonym dostępem albo flagę strojenia \`VLLM_*\`. Obowiązują tylko przy wyborze niestandardowym, więc wybranie gotowej konfiguracji je usuwa.
...
- Les arguments de démarrage **Personnalisés** sont désormais entre guillemets comme dans un shell, si bien que \`--foo "a b"\` et \`--bar '{"k": 1}'\` restent chacun un seul argument. Auparavant la chaîne était découpée sur les espaces et une valeur entre guillemets ne survivait pas. Seuls les guillemets sont interprétés — une \`$VAR\`, un motif \`*\`, un tube ou une redirection parvient à vLLM tel quel.
- L'option de modèle **Personnalisé** accepte désormais une liste de variables d'environnement en plus des arguments de démarrage — un \`HF_TOKEN\` pour un modèle à accès restreint, ou un indicateur de réglage \`VLLM_*\`. Elles ne s'appliquent qu'à une sélection personnalisée ; choisir un préréglage les efface.
