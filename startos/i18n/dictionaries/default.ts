...
const dict = {
  ...
  'Extra environment variables for the `vllm serve` process — a HuggingFace token for a gated model, or a `VLLM_*` tuning flag. The package sets `HF_HUB_CACHE`, `PYTHONUNBUFFERED` and `HF_HUB_VERBOSITY`, and a variable named here replaces the one it sets.': 42,
  'Pointing `HF_HUB_CACHE` somewhere other than `/data/models` moves the model cache off the persistent volume, so every start re-downloads the model.': 43,
  Name: 44,
  'May contain letters, digits and underscores, and may not start with a digit.': 45,
  Value: 46,
  'Qwen3.6 35B-A3B': 24,
  'Qwen3.6 27B': 26,
